...
class coordendas:
  def __init__(self,archivo_entrada,inicio):
    self.kml_file=archivo_entrada
    self.inicio=inicio

  def extraer_decenas(self,cadena):
    cadena_numerica = ''.join(filter(str.isdigit, cadena))
    if len(cadena_numerica) >= 2:
        self.decenas = cadena_numerica[-1]
        return int(self.decenas)
    else:
        return None

  def convertir_a_utm(self,latitud, longitud):
    p1 = Proj(init='epsg:4326')
    p2 = Proj(init='epsg:32618')
    self.este, self.norte = transform(p1, p2, longitud, latitud)
    return self.este, self.norte

  def convertir_a_geo(self,utm_zone, utm_band,x,y):
    in_proj = Proj(proj='utm', zone=utm_zone, ellps='WGS84', south=(utm_band==True))
    out_proj = Proj(proj='latlong', datum='WGS84')
    self.lon, self.lat = transform(in_proj, out_proj, x, y)
    return self.lon, self.lat

  def extract_coordinates(self,kml_file):
    coordinates_array=[]   
    tree=ET.parse(kml_file)
    root=tree.getroot()
    for coordinates in root.iter('{http://www.opengis.net/kml/2.2}coordinates'):
      coord=coordinates.text.strip().split(' ')
      for elemento in coord:
        coord=elemento.split(',')
        coordinates_array.append(coord)
    return coordinates_array

  def main(self,ruta_salida):
    utm_zone = 18
    utm_band = 'S'
    coordinates_array=self.extract_coordinates(self.kml_file)
    ...
    for i in range(len(coordinates_array)):
      if i>0:
        ...
        este_1, norte_1 = self.convertir_a_utm(lat_1,lon_1)
        este_2, norte_2 = self.convertir_a_utm(lat_2,lon_2)
        ...
        for n in range(cantidades):
          ...
          coord_final=self.convertir_a_geo(utm_zone, utm_band,este,norte)
    ...
    with open(ruta_salida,"w") as f:
      ...
      f.write('''  <Folder>
        <name>postes</name>\n''')
      f.write('    <Placemark>\n')
      f.write('      <name>'+ str(self.inicio) +'</name>\n')
      ...
      for coordenadas in coordenadas_finales:
        ...
        if (100*i) % 1000==0:
          f.write('    <Placemark>\n')
          f.write('      <name>'+ str(self.inicio+i/10)+str(000)+'</name>\n')
          f.write('''      <styleUrl>#msn_triangle0</styleUrl>
          <Point>\n''')
